    ...
    def MailParsingGeneral._get_attachment_text_internal_one_file(content_type, body, charset = 'utf-8')
        ...
        if content_type == 'text/plain'
            ...
            if content_type == 'application/vnd.ms-word'
                ...
                # catdoc on RTF produces less comments and extra bumf than --text option to unrtf
                AlaveteliExternalCommand.run("catdoc", tempfile.path, default_params)
                ...
                xml = AlaveteliExternalCommand.run("/usr/bin/unzip", "-qq", "-c", tempfile.path, "word/document.xml", {:binary_output => false})
                if !xml.nil?
