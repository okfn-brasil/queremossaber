...
class IncomingMessage < ActiveRecord::Base
    ...
    def _calculate_valid_to_reply_to
        ...
        if MailParsing.empty_return_path?(self.mail)
            return false
        end
        if !MailParsing.get_auto_submitted(self.mail).nil?
            return false
        end
        return true
    end
    ...
    def extract_attachments!
        force = true
        attachment_attributes = MailParsing.get_attachment_attributes(self.mail(force))
        attachment_ids = []
        ...
        if !main_part.nil?
            ...
            for uudecode_attachment in uudecoded_attachments
                ...
                attachment_ids << uudecode_attachment.id
            end
        end

        # now get rid of any attachments we no longer have
        if attachment_ids.empty?
            FoiAttachment.destroy_all("incoming_message_id = #{self.id}")
        else
            FoiAttachment.destroy_all("id NOT IN (#{attachment_ids.join(',')})
                                      AND incoming_message_id = #{self.id}")
        end
   end
    ...
    def _get_attachment_text_internal
        ...
        for attachment in attachments
            text += MailParsingGeneral._get_attachment_text_internal_one_file(attachment.content_type, attachment.body, attachment.charset)
        end
